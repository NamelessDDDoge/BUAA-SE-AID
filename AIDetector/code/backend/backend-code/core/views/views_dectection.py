...
def submit_detection2(request):
    ...
    try:
        detection_task, _image_uploads = create_image_detection_task(
            ...
            llm_model_name=request.data.get('llm_model_name', None),
...
def create_resource_task(request):
    ...
    paper_text_override = request.data.get('paper_text_override')
    review_text_override = request.data.get('review_text_override')
