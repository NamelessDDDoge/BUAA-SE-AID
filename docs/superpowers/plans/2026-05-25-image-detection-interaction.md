# Image Detection Interaction Chain — Test Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write integration tests that reproduce bugs in the image detection interaction chain (upload→detect→results→report), covering multi-image ordering, quota refund, and status consistency.

**Architecture:** Two-tier testing — fast (mock get_result) and bridge (fake subprocess AI service). All tests use pytest + django_db. No bug fixes — tests assert correct behavior to expose failures.

**Tech Stack:** pytest, Django TestCase (migrated to pytest style), DRF APIClient, unittest.mock.patch, numpy, PIL
...
**Target test file:**
`AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`

**Run all tasks with:**
```
cd AIDetector/code/backend/backend-code && python -m pytest core/tests/integration/api/detection/test_image_detection_interaction_chain.py -v
```

**Standard patch stack (fast tier):**
```python
@patch("core.services.capabilities.image.local_detection.get_result", return_value=fake_detection_payload())
@patch("core.services.capabilities.image.local_detection.generate_detection_task_report", return_value="reports/x.pdf")
...
@patch("core.views.views_dectection._start_detection_task_thread",
       side_effect=lambda task_id, image_ids, if_use_llm, num_images: _run_detection_task_async(task_id, image_ids, if_use_llm, num_images))
```

**Always use `if_use_llm=False` in all detection submissions.**

**Auth pattern:** `APIClient().force_authenticate(user)`

**Factory imports:**
```python
from core.tests.factories import (
    make_user, make_organization, make_file_management,
    make_detection_task, make_detection_result, make_image_upload,
)
```

---

## Shared Helpers (top of test file)
...
## Task 1: Complete Upload → Extract → Detect → Results Chain (Fast Tier, Single Image)

**Interaction being tested:** The full end-to-end flow through five distinct API calls in sequence.
Each step depends on data produced by the previous step. A bug in any hand-off (e.g., wrong image ID
returned by extract, or result not linked to the correct task) will cause a downstream assertion to fail.

**Files:**
- Create: `AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`
...
```
cd AIDetector/code/backend/backend-code && python -m pytest "core/tests/integration/api/detection/test_image_detection_interaction_chain.py::TestFullUploadDetectResultsChain::test_full_chain_single_image_all_hand_offs_correct" -v
```

Expected: **PASS** — confirms the nominal single-image chain works end-to-end. If it fails, the hand-off between any two steps is broken and must be investigated before proceeding.

- [ ] **Step 3: Commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): Task 1 — full upload→detect→results chain (fast tier, single image)"
```

---

## Task 2: Multi-Image Batch With Ordering Verification (Fast Tier)

**Interaction being tested:** When two images are submitted together, the pipeline assigns detection
results back to the correct image. The bug scenario: if the ordering of `DetectionResult` records
in the database differs from the ordering of images in the AI payload, result[0] maps to image 2
instead of image 1 — a silent data corruption bug.

**Files:**
- Modify: `AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`

- [ ] **Step 1: Append the ordering test class**
...
```
cd AIDetector/code/backend/backend-code && python -m pytest "core/tests/integration/api/detection/test_image_detection_interaction_chain.py::TestMultiImageBatchOrdering" -v
```

Expected: These tests may **FAIL** if the ordering bug exists (results assigned to wrong images). A failure on `assert self.image1.isFake is False` means result index 0 in the payload was incorrectly assigned to image_2 instead of image_1.

- [ ] **Step 3: Commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): Task 2 — multi-image batch ordering verification (fast tier)"
```

---

## Task 3: Detection Failure → Quota Refund → Task List Shows Failed (Fast Tier)

**Interaction being tested:** When the AI pipeline raises an exception, three side-effects must happen
atomically and consistently: (1) the task is marked `failed`, (2) the quota is refunded, and (3) the
user-facing task list endpoint reflects the failure. A bug in any of these three interactions (e.g.,
quota consumed but not refunded, or task list showing wrong status) is caught here.

**Files:**
- Modify: `AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`

- [ ] **Step 1: Append the quota refund test class**
...
```
cd AIDetector/code/backend/backend-code && python -m pytest "core/tests/integration/api/detection/test_image_detection_interaction_chain.py::TestDetectionFailureQuotaRefund" -v
```

Expected: `test_quota_is_refunded_when_pipeline_fails` may **FAIL** if the refund path is not triggered on `get_result` returning `None` (vs. raising an exception). `test_all_result_endpoints_consistently_show_failed_after_pipeline_error` should **PASS** based on existing behavior.

- [ ] **Step 3: Commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): Task 3 — failure quota refund and status consistency (fast tier)"
```

---

## Task 4: Repeated Detection on the Same Image

**Interaction being tested:** What happens when a user submits detection on an image that has already
been detected? The system creates separate `DetectionTask` + `DetectionResult` records per submission.
This test checks: (a) the second submission is accepted, (b) `DetectionResult` objects are correctly
scoped to their respective tasks (not merged), (c) `ImageUpload.isDetect` reflects the latest state,
and (d) the status endpoint for each task is independent.

**Files:**
- Modify: `AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`

- [ ] **Step 1: Append the repeated detection test class**
...
```
cd AIDetector/code/backend/backend-code && python -m pytest "core/tests/integration/api/detection/test_image_detection_interaction_chain.py::TestRepeatedDetectionOnSameImage" -v
```

Expected: `test_second_submission_creates_independent_task_and_result` may **FAIL** if the second submission is rejected (400/404) or if both tasks share a single `DetectionResult` record (get_or_create collision). `test_user_tasks_list_shows_both_submissions` should **PASS** if submission is accepted.

- [ ] **Step 3: Commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): Task 4 — repeated detection on same image, task isolation"
```

---

## Task 5: Bridge Tier — Full Pipeline Using Fake Subprocess

**Interaction being tested:** The real subprocess path through `_run_local_inference` → `_run_local_detection_batch` → `_persist_detection_result`. The fake entrypoint script (`fake_ai_service_entrypoint.py`) executes as a real Python subprocess and writes a deterministic payload. This tier catches bugs in the subprocess launch, stdout parsing, batch zip packaging, and result persistence that mocks cannot catch.

**Files:**
- Modify: `AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`

- [ ] **Step 1: Append the bridge tier test class**
...
```
cd AIDetector/code/backend/backend-code && python -m pytest "core/tests/integration/api/detection/test_image_detection_interaction_chain.py::TestBridgeTierFullPipeline::test_bridge_two_images_results_assigned_correctly_by_zip_ordering" -v
```

Expected: **PASS** if the subprocess ordering is correct. **FAIL** if `bridge_image_1.isFake is True` — indicating that the real subprocess path maps results to images differently than the fast-tier mocks.

- [ ] **Step 3: Commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): Task 5 — bridge tier full pipeline with fake subprocess"
```

---

## Task 6: Concurrent Task Status Polling — is_running Flag Consistency

**Interaction being tested:** The `is_running` flag in the status endpoint is derived from
`task.status in {"pending", "in_progress"}`. This test checks: (a) `is_running=True` during the
window when the task has been submitted but the thread has not yet started, (b) `is_running=False`
once completed, and (c) `is_running=False` on failed tasks. Also verifies that the `detection_results`
list in the status payload is populated during the pending state (not just after completion).

**Files:**
- Modify: `AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`

- [ ] **Step 1: Append the status consistency test class**
...
```
cd AIDetector/code/backend/backend-code && python -m pytest "core/tests/integration/api/detection/test_image_detection_interaction_chain.py::TestTaskStatusConsistency" -v
```

Expected: All three tests should **PASS**. If `test_is_running_true_while_task_is_pending` fails on `detection_results` being empty, that indicates the status endpoint does not include pending-state `DetectionResult` records.

- [ ] **Step 3: Commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): Task 6 — concurrent task status polling, is_running consistency"
```

---

## Task 7: Report Download Endpoint After Detection Completes

**Interaction being tested:** The report download endpoint at `GET /api/tasks/<task_id>/report/`
requires the task to be `completed` AND a report file to exist on disk. This tests the interaction
chain: detection completes → report file is generated (mocked path saved on task) → download endpoint
serves the file. Key bugs: endpoint returns 400 if `task.report_file` was not saved after generation,
or returns 500 if `ensure_task_report_file` regenerates to a path that does not exist.

**Files:**
- Modify: `AIDetector/code/backend/backend-code/core/tests/integration/api/detection/test_image_detection_interaction_chain.py`

- [ ] **Step 1: Append the report download test class**
...
```
cd AIDetector/code/backend/backend-code && python -m pytest "core/tests/integration/api/detection/test_image_detection_interaction_chain.py::TestReportDownloadAfterDetection" -v
```

Expected: `test_report_download_returns_400_for_pending_task` should **PASS**. `test_report_download_returns_200_after_completed_detection` and `test_image_level_report_download_via_tasks_image_endpoint` may **FAIL** if `ensure_task_report_file` raises a `FileNotFoundError` when the report file does not physically exist on disk.

- [ ] **Step 3: Commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): Task 7 — report download endpoint after detection completes"
```
...
```
cd AIDetector/code/backend/backend-code && python -m pytest core/tests/integration/api/detection/test_image_detection_interaction_chain.py -v --tb=short 2>&1 | tee /tmp/chain_test_results.txt
```

- [ ] **Count pass/fail and document which tests expose bugs**

Any test that fails is a candidate for a bug fix PR. The failures should be documented in the task tracker / commit message, NOT fixed in the same PR as this test file.

- [ ] **Final commit**

```
git add core/tests/integration/api/detection/test_image_detection_interaction_chain.py
git commit -m "test(chain): complete image detection interaction chain test plan (Tasks 1-7)"
```
...
| Task | Test | Likely Failure Mode |
|------|------|---------------------|
| T2 | `test_result_for_image1_is_authentic_and_image2_is_tampered` | `DetectionResult` DB insertion order != AI payload index order |
| T3 | `test_quota_is_refunded_when_pipeline_fails` | `get_result=None` may not trigger the exception branch that calls `_refund_detection_usage` |
| T4 | `test_second_submission_creates_independent_task_and_result` | `get_or_create` collision creates only 1 `DetectionResult` for 2 submissions |
| T5 | Bridge ordering test | Real subprocess filename sorting differs from mock payload key ordering |
| T7 | `test_report_download_returns_200_after_completed_detection` | `ensure_task_report_file` raises when PDF not on disk |
